...
def qsub_submit(command_filename, hold_jobid = None, fname = None):
	...
	jobid = out.split(' ')[2]
	return int(jobid)
path = '/netapp/home/idriver/01272015_hu'
out= '${TMPDIR}'
...
result_file_name = 'results_av_01272015_hu'
pathlist = []
...
for ind_num, p in enumerate(pathlist):
	...
	tophat_cmd = 'tophat2 -p 8 -r 50 -G '+annotation_file+' --transcriptome-index=/netapp/home/idriver/transcriptome_data_hg19_ERCC/known_E -o '+result_file+' '+index_gen_loc+' '+final_files
...
date
""" % vars()
	if name == 'Lane1_CAGAGAGGAGAGTAGA_1':
		filename = '%s.sh' % name
