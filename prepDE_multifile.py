...
samples = []
for root, dirnames, filenames in os.walk(opts.input):
    if 'transcripts.gtf' in filenames:
        root_list = root.split('/')
        print(os.path.join(root_list[-2],root_list[-1]))
        samples.append(os.path.join(root_list[-2],root_list[-1]))
if len(samples) == 0:
  parser.print_help()
  print " "
...
for s in samples:
    ...
    try:
        with open(glob.iglob(os.path.join(opts.input,s,"transcripts.gtf")).next()) as f:
...
for q, s in enumerate(samples):
    ...
    try:
        with open(glob.iglob(os.path.join(opts.input,s,"transcripts.gtf")).next()) as f: #grabs first .gtf file it finds inside the sample subdirectory
