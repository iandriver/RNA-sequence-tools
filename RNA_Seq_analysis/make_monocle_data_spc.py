...
path_to_file = '/Volumes/Seq_data/cuffnorm_m_spc_d0_4_7_new'
#name of file containing gene
gene_file_source = 'go_search_genes_lung_all.txt'

base_name = 'm_spc_d0_4_7_new'
#load file gene
...
def make_new_matrix(org_matrix_by_cell, gene_list_file):
    ...
    cell_data = []
    cell_label_dict ={'sca':('Sca', 0, 'ctrl'), 'spc_ctrl':('D0',0, 'ctrl'), 'spc_d4':('D4',4, 'pnx'), 'spc_d7':('D7', 7, 'pnx')}
    new_cell_list = []
    old_cell_list = []
    for cell in cell_list:
        ...
        if cell[0] == 'C':
            k = 'spc_d7'
            tracking_id = 'd7_'+cell
            match = True
            num = int(cell.strip('C_'))
            old_cell_list.append(cell)
            new_cell_list.append(tracking_id)
        elif cell[0:8] == 'spc_ctrl':
            k = 'spc_ctrl'
            tracking_id = cell
