run_monocle <- function(dir){
	...
	saveRDS(my_data, "monocle_data_js1.rds")
	...
	pdgfra_marker_genes <- row.names(subset(fData(my_data), GeneID %in% c('Dcn', 'Serpinf1', 'Lmna', 'Serping1', 'Cxcl1', 'Lum', 'Sod1', 'Apoe', 'thbs2', 'Has1', 'Has2', 'Notch2', 'Col5a3', 'Dpt', 'Tcf21')))
	...
	diff_test_res_cond <- differentialGeneTest(my_data[pdgfra_marker_genes,], fullModelFormulaStr="expression~Pdgfra")
	diff_test_res_nkx <- differentialGeneTest(my_data[pdgfra_marker_genes,], fullModelFormulaStr="expression~timepoint")
	sig_genes_cond <- subset(diff_test_res_cond, qval < 0.1)
	print(sig_genes_cond)
	sig_genes_nkx <- subset(diff_test_res_nkx, qval < 0.1)
	print(sig_genes_nkx)
	sig_genes_cond <- merge(fData(my_data), sig_genes_cond, by="row.names")
	sig_genes_nkx <- merge(fData(my_data), sig_genes_nkx, by="row.names")
	sig_genes_cond[,c("GeneID", "pval", "qval")]
	sig_genes_nkx[,c("GeneID", "pval", "qval")]
	diff_test_res <- differentialGeneTest(my_data[expressed_genes,], fullModelFormulaStr="expression~timepoint")
	...
	my_data <- orderCells(my_data, num_paths=3, reverse=TRUE)
	pdf('spanning_tree_plot.pdf', width=8, height=14)
	...
	print(span_tree)
	dev.off()
	pdf('marker_genes_psuedotime.pdf', width=8, height=14)
	...
	print(pstime)
	dev.off()
	saveRDS(my_data, "monocle_data_js2.rds")
