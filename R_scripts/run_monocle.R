run_monocle <- function(dir){
	...
	saveRDS(my_data, "monocle_data_ips_GFP1.rds")
	...
	diff_test_res_cond <- differentialGeneTest(my_data[marker_genes,], fullModelFormulaStr="expression~cell_type")
	diff_test_res_nkx <- differentialGeneTest(my_data[marker_genes,], fullModelFormulaStr="expression~nkx")
	...
	diff_test_res <- differentialGeneTest(my_data[expressed_genes,], fullModelFormulaStr="expression~nkx")
	...
	pstime <- plot_genes_in_pseudotime(my_data[marker_genes,], min_expr = NULL, cell_size = 0.75, nrow = NULL, ncol = 4, panel_order = NULL, color_by = "State", trend_formula = "adjusted_expression ~ sm.ns(Pseudotime, df=3)", label_by_short_name = TRUE)
	print(pstime)
	dev.off()
	saveRDS(my_data, "monocle_data_ips_GFP2.rds")
