...
path = '/netapp/home/idriver/pengt_2nd'
genome = 'mouse'
if genome == 'human2':
    ...
    rsem_ref = '/netapp/home/idriver/rsem_h38/GRCh38'
if genome == 'human1':
    ...
    if dirs == ['fastqc'] or dirs ==[]:
        ...
        for f in files:
            if '_trimed_fastq' in f:
                ...
                final_files+=os.path.join(root,f)
        if not single_end:
            command = 'rsem-calculate-expression --bowtie2 -p 10 --paired-end '+final_files+' '+rsem_ref+' '+out
        else:
            command ='rsem-calculate-expression --bowtie2 -p 10 '+final_files+' '+rsem_ref+' '+out
...
#$ -r y
#$ -j y
#$ -l netapp=10G,scratch=10G,mem_total=20G
#$ -pe smp 10
#$ -R yes
#$ -l h_rt=12:59:00
set echo on
...
""" % vars()
        if name != 'A01_1st':
            ...
            out_com, err = process.communicate()
            print(out_com)
            run_1 = False
