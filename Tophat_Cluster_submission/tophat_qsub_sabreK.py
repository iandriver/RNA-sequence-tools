...
path = '/netapp/home/idriver/Kidney_sabre'
out= '${TMPDIR}'
...
#$ -l arch=linux-x64
#$ -S /bin/bash
#$ -o /netapp/home/idriver/results_Kidney_sabre_mm10
...
cd $TMPDIR
mkdir %(name)s
mkdir -p /netapp/home/idriver/results_Kidney_sabre_mm10/%(name)s
...
cd $TMPDIR
cp -r %(name)s/* /netapp/home/idriver/results_Kidney_sabre_mm10/%(name)s
rm -r %(name)s

date
  """ % vars()
  if cell_number != 10:
    filename = 'Kidney_Sample-C%d.sh' % cell_number
    write_file(filename, contents)
    print tophat_cmd
    print cufflinks_cmd
    print cuffquant_cmd
    jobid = qsub_submit(filename, name = 'Kidney_Sample_C%d' % cell_number)
