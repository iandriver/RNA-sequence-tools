...
path = '/netapp/home/idriver/141203_D00108_0278_AHBA86ADXX_Project_werbz-SC-Lib2'
out= '${TMPDIR}'
...
for p in pathlist:
  ...
  tophat_cmd = 'tophat2 -p 8 -r 50 -G '+annotation_file+' --transcriptome-index=/netapp/home/idriver/transcriptome_data_hg19/known_hu -o '+result_file+' '+index_gen_loc+' '+final_files
...
#$ -l arch=linux-x64
#$ -S /bin/bash
#$ -o /netapp/home/idriver/results_hg19_Lib2
...
cd $TMPDIR
mkdir %(name)s
mkdir -p /netapp/home/idriver/results_hg19_Lib2/%(name)s
...
cd $TMPDIR
cp -r %(name)s/* /netapp/home/idriver/results_hg19_Lib2/%(name)s
rm -r %(name)s

date
  """ % vars()
  if name != 'A1' :
    filename = '%s_Lib2.sh' % name
    write_file(filename, contents)
    print tophat_cmd
    print cufflinks_cmd
    print cuffquant_cmd
    jobid = qsub_submit(filename, name = '%s_Lib2' % name)
